...
const findArticlesLimited = (startingArticle, endingArticle,
  textSimilarityPercent, callback) => {
   ...
   var startTitle = startingArticle.articleTitle;
   endTitle = endingArticle.articleTitle;
   var startArticleText = tokenizer.tokenize(
     normalizeDocumentText(startingArticle)
   );
   articleDv = new DocumentVector(startArticleText);

   visited[startTitle] = 1;
   queue.push(startTitle);
   processed.push(startingArticle);
   textSimilarity = normalizePercentage(textSimilarityPercent);

   findArticles(() => {
     processed = processed.filter((x) => {
       if (x.articleTitle === endTitle) {
         return true;
       }
...
const findArticles = (callback) => {
  ...
  // added to the final processed array.
  .then((results) => {
    ...
    if (currentArticleTitle === endTitle) {
      return callback();
    }
